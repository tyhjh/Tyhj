...
public class VmyEssay extends Activity{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.myessay);
		Intent in=getIntent();
		...
		query.findInBackground(new FindCallback<AVUser>() {
			...
			public void done(List<AVUser> arg0, AVException e) {
				if(e==null&&arg0.size()>0){
					 Vuser=arg0.get(0);
					 dp = new VEssayAdapter(VmyEssay.this, R.layout.list_item, essayList);
						init();
						list1.setAdapter(dp);
				}
			}
	...
	private void init() {
		WindowManager wm = (WindowManager) VmyEssay.this
	...
	int xxx=(int) (height/25.7);
		...
		tc.touch(publish);
		tc.touch(back);
		who.setText(Vuser.getString("pName")+"的说说");
		back.setOnClickListener(new OnClickListener() {
			
			@Override
			public void onClick(View v) {
				Intent in=new Intent(VmyEssay.this,Visit.class);
				startActivity(in);
				VmyEssay.this.finish();
			}
		});
		publish.setOnClickListener(new OnClickListener() {
			...
			@Override
			public void onClick(View v) {
				Intent in=new Intent(VmyEssay.this,Publish.class);
				startActivity(in);
				VmyEssay.this.finish();
			}
		});
		...
		Picasso.with(VmyEssay.this).load(R.drawable.leftgo).resize(xxxx, xxxx)
		...
		swiperefresh.setOnRefreshListener(new OnRefreshListener() {
			...
			public void onRefresh() {
				...
				new Handler().postDelayed(new Runnable() {
					@Override
					public void run() {
						Intent in = new Intent(VmyEssay.this,VmyEssay.class);
						in.putExtra("usernumber", usernumber);
						VmyEssay.this.finish();
						startActivity(in);
						VmyEssay.this.overridePendingTransition(R.anim.out, R.anim.enter);  
						// 刷新完成
		...
		query2.whereEqualTo("username", Vuser.getString("username"));
					...
					for (int i = 0; i < avObjects.size(); i++) {
						try {
							Essay essay = new Essay(avObjects.get(i).getString(
									"pName"), avObjects.get(i).getAVFile("headImage").getUrl(), avObjects.get(i).getAVFile("EssayImage").getUrl(),
